# Onion_Trime_Files（手機 同文輸入法 注音、雙拼 洋蔥 方案）

####  ※ 請勿使用於商業營利相關行為
####  ※ Commercial use is prohibited
####  ※ 目前只測試 trime 3.2.1 版，其他版本未測
### 內容說明：

- trimefiles 中包含五個主方案（三個注音、一個注音雙拼、二個形碼）和一眾掛接方案
...
- 產生的『洋蔥手機同文方案_{當天日期}』該資料夾內，會把各方案所須使用的文件，分別安放在該『方案名稱』資料夾，安裝部署時，把內含文件通通放入『 rime 』安裝資料夾中（預設已有 opencc 資料夾，移動方案 opencc 裡面檔案到手機 rime/opencc ），「重新部署」即可。
